...
3. Commit only the experiment's code to it, staging by explicit path; never commit ${DIR} there. Build what a finding rests on from a committed state, so the finding can name the commit. Commit messages follow ${core('conventions/commit-messages.md')}. The documentation routing above does not apply to this branch: nothing on it lands.
4. You may build, run tests and run the app on a simulator, an emulator or as a local process — never on a physical device, which may hold real data. Use a simulator or emulator created for this experiment, never one that already holds this app, whose data may be the developer's, and name it under ### Experiment. Drive the app through the project's driver, resolved the way this platform's validator resolves it (${core('conventions/driver-contract.md')}). This run's drive_app is ${DRIVE_APP}; at off, drive nothing.
