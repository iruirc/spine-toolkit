...
For BUG a build and a full test run are both mandatory, through this platform's own build and test tooling, and so is mobile MCP regardless of which layer changed — you replay the reproduction scenario from Reproduce.md against a running instance of the app. Validation is not PASSED without your own explicit statement that the bug no longer reproduces. If mobile_mcp resolves to off — Task.md [MOBILE_MCP] first, then CLAUDE-spine-toolkit.md ## Validation — you do not replay it: return reproduction_status deferred-manual, put the replay steps into ${DIR}/ManualChecks.md and their titles into manual_checks, and claim nothing about whether the bug is fixed. Reserve not-replayed for a replay that was expected of you and stayed inconclusive.
