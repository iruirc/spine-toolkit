...
Cover the focused fix${A.need_test === false ? '' : ', a regression test that locks in the scenario from Reproduce.md'}, and any migration or compatibility step the change forces.${A.need_test === false ? ' This task owes no test (need_test=false): the plan has no phase of kind test and no regression-test item.' : ''} Every phase has to end independently buildable, green, and committable on its own.
...
// ── Fix ─────────────────────────────────────────────────────────────────────
if (runs('Fix')) {
  if (!(A.need_test === false ? need('Fix', 'developer') : need('Fix', 'developer', 'tester'))) return finish('ask_user')
  ...
  const phasesDone = await runPhases(
    ...
    A.need_test === false
      ? 'Commit type: fix for the repair itself, chore for build or config only. This task owes no test (need_test=false): the phase writes none.'
      : 'Commit type: fix for the repair itself, test for the regression-test phase, chore for build or config only. A regression test is mandatory for this profile — it is what stops the bug coming back.',
  )
