...
const DOCS_NOTE = A.docs === 'off' || A.docs === false ? '' : `Documentation: when this stage changes files, run "<core root>/scripts/docs-route.sh route <project root> --task-dir ${DIR} --phase <phase>" with the change set on stdin — git diff --name-status for what this phase landed, or, at Plan, the paths the plan intends to touch. Every path is relative to the project root, so a diff taken inside a checkout goes through "docs-route.sh reorigin <checkout>" first and the concatenation is what you feed. Answer every row it opens in ${DIR}/Docs.md by applying the spine-toolkit:docs-route skill. Before you commit, run the same script with "check" instead of "route" and the same change set: exit 1 means a blocking question is still open and the phase does not close; exit 2 means the registry or the table itself is malformed — fix that, it is not a question anyone can answer. At Done also run "audit", then "tracker": "audit" names components living away from their coverage and files created outside every covers, both advisory and neither stops anything; "tracker" regenerates the step table of every declared tracker between its markers, leaving everything outside them alone; a tracker whose markers are malformed is refused rather than reshaped, and named. At Review run "check" with the task's whole change set and no --phase: it names every row still open across all phases. Report them; Review does not enforce them. The core root is the directory holding workflows/. All of it is skipped by a stage that changes no files, by a task whose Task.md carries [DOCS] = [off], by a project whose ## Docs block carries enabled: off, and by a project that declares no components at all — neither in the map named by ## Docs → map, DocsMap.md by default, nor in any package it holds.
