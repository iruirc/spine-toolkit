...
if (runs('Write')) {
  ...
  if (!plan) plan = FIX_REVIEW ? FIX_PLAN : await readPlan('Write', 'tester')
  ...
  const phasesDone = await runPhases(
    ...
    fixGuidance('Commit type: test for a phase that adds test logic, chore for a test-infrastructure-only phase (fixtures and helpers with no test logic of their own). A phase whose checks were never run is not green, it is unknown.'),
  )
...
if (runs('Review') && A.need_review !== false) {
  ...
  review = await agent(
    brief(
      'Review',
      `${reviewScope('Review the TESTS this task added — not the production code.')}${RANGES ? ' Judge only the tests in them, not the production code.' : ''} Write ${DIR}/Review.md. Its FIRST LINE is required to be exactly:
...
if (runs('Done')) {
  ...
  const done = await agent(
    ...
    { label: 'done', phase: 'Done', agentType: A.agents.tester, schema: DONE_ARTIFACT, ...tuning('tester', 'done') },
  )
