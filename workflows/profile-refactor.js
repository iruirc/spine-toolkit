...
if (runs('Refactor')) {
  ...
  if (!plan) plan = FIX_REVIEW ? FIX_PLAN : await readPlan('Refactor', 'refactorer')
  ...
  const phasesDone = await runPhases(
    ...
    fixGuidance('Commit type: refactor for a structural phase, test for a test-only phase, chore for build or config only. External behaviour does not change — if a pre-existing test needs editing to pass, that is a signal you changed behaviour, so stop and say so rather than editing the test.'),
  )
...
if (runs('Done')) {
  ...
  const done = await agent(
    ...
    { label: 'done', phase: 'Done', agentType: A.agents.refactorer, schema: DONE_ARTIFACT, ...tuning('refactorer', 'done') },
  )
