...
@test "a progress component named in DOCS_NEW is named but never asked" {
  task; two_components
  printf '[TASK_TYPE] = [FEATURE]\n[DOCS_NEW] = [Restock-Progress:progress]\n' >"$TASK/Task.md"
  run bash -c "printf 'M\tSources/Export/Writer.txt\n' | '$DR' route '$PROJ' --task-dir '$TASK' --phase 3"
  [ "$status" -eq 0 ]
...
t = re.sub(r'^\| 3 \| Ledger \| state \| (\S+) \|  \|  \|$',
           lambda m: '| 3 | Ledger | state | %s | %s | %s |' % (m.group(1), v, n),
           t, flags=re.M)
...
@test "Applicable without a touched file under places does not close it" {
  ...
  run bash -c "printf 'M\tSources/Ledger/Resolver.txt\n' | '$DR' check '$PROJ' --task-dir '$TASK' --phase 3"
  [ "$status" -eq 1 ]
  case "$output" in *"Documents/Ledger/"*) ;; *) echo "$output"; return 1 ;; esac
}

@test "Applicable with a file under places changed in the same range closes it" {
  task; two_components; routed
  verdict "Applicable" "wrote the resolution rule"
  run bash -c "printf 'M\tSources/Ledger/Resolver.txt\nM\tDocuments/Ledger/Resolution.md\n' | '$DR' check '$PROJ' --task-dir '$TASK' --phase 3"
  [ "$status" -eq 0 ]
}
