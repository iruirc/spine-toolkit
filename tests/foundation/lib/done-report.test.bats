...
@test "the calibration row of a task is corrected in place, in the skill and in both briefs" {
  ...
  grep -qF '`task_id` names the task by its number' "$est" \
    || { echo "feature-estimation does not define task_id"; return 1; }
  grep -qF "Record this feature's data point in the calibration log: correct this task's row — the one whose task_id names this task, as feature-estimation's Column semantics defines it — if the log already has one, otherwise append it." "$ROOT/workflows/profile-feature.js" \
    || { echo "the FEATURE Done brief still appends"; return 1; }
  grep -qF "Record this epic's data point in the calibration log: correct its row — the one whose task_id names this epic, as feature-estimation's Column semantics defines it — if the log already has one, otherwise append it." "$ROOT/workflows/profile-epic.js" \
