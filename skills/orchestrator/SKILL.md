...
|---|---|---|---|
| `task_id` | string | NL/$ARGUMENTS (например `026`, `052`, `001-foo`) | **обязательно** — ошибка с подсказкой `"Укажите номер задачи: 'запусти 026'"` |
...
1. **Инициализация проекта** — "создай проект" / "инициализируй" / отсутствуют `.xcodeproj` и `Package.swift` → напомнить про агента `swift-init`. Оркестратор не запускается.
