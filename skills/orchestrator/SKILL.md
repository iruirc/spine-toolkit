...
`drive_app` — whether the Validation stage may drive the running app through the platform's own tooling. Resolved by the same run of `resolve-settings.sh json`; `conventions/task-settings.md` holds the chain and field table. Always filled, for every profile. `auto` leaves the choice to the profile named in `## Validation`'s parenthetical; `off` is the project saying it has nothing to drive. The Driver pre-flight (**Routing**, check 4) reads this field to decide whether it runs at all.
...
<what happened>`, `<source>` being `Task.md [FIELD]` or a config path and `## <Block>`. The source
says which field, the text after the quotes which key:

| text after the quotes | source names | key |
|---|---|---|
| `is the pre-depth spelling, read as '<depth>'` | `[WALKTHROUGH]` or `## Reporting` | `warn_walkthrough_pre_depth` |
| `not recognized, skipped` | `[WALKTHROUGH]` or `## Reporting` | `warn_walkthrough_unrecognised` (`{value}`) |
| `not recognized, skipped` | `## Budgets` | `warn_budget_unrecognised` (`{line}`) |
| `not recognized, skipped` | `[MODELS]`, `[EFFORT]`, `## Models` or `## Effort` | `warn_tuning_unrecognised` (`{source}`, `{entry}`) |
