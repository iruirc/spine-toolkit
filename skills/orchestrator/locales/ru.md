...
Драйвер `{driver}` назван в конфигурации проекта, но его манифест не резолвится — плагин не
...
Драйвер `{driver}` установлен, но инструментов с префиксом `mcp__{namespace}__` в этой сессии нет —
его MCP-сервер не подключён. Валидация отдаст свои UI-проверки вам вместо того, чтобы гнать
приложение. Запустите сервер или напишите `driver: —` в `## Validation`, если так и задумано.
