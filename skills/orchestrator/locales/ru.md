...
## budget_over_limit
`{artifact}` — {actual} строк при потолке в {cap}. Сокращаю до потолка, не теряя требований: логи и дампы идут ссылкой, а не вставленным текстом.

## task_doc_anchor_missing
`{step}`: в `Task.md` нет текста под `{anchor}`. Возвращаю архитектору: заполнить якорь или написать `— <причина>`, если он неприменим.
