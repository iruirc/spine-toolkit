...
field is absent**, which is not the same thing as the template's line. Eighteen agree; `[SCALE]` does
...
line for eighteen of them, `full` for `[SCALE]` — and listed in `filled_default_fields`. The moved
