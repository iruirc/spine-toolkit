...
Every retrospective MUST record one row in the project's calibration log at **`Tasks/_calibration/estimation-log.md`** (create the file with a header if absent). One row per finished feature/epic: date, task id, estimated range, actual (human ?? proxy with its source label), and — in AI-assisted mode — the per-class observed divisors. A Done that runs again corrects its own task's row instead of appending a second, and folds any duplicate rows an older release left for the same task into that one row. This file is the durable evidence base; a single `## Estimate retrospective` in one `Done.md` is not enough to calibrate against after it scrolls out of context.
...
Column semantics: `task_id` names the task by its number — `041` or `041-photo-feed` — and a step by its epic's number and its own — `042-03` or `042-cart/03-promo` — because step folders are numbered afresh in every epic; a task's own row is the one naming the same task, whatever slug follows the number. `type` ∈ {FEATURE, EPIC, BUG, REFACTOR, TEST, RESEARCH}; `posture` ∈ {human, ai-assisted}; `estimated_range` is the named best–worst range as reported; `source` ∈ {human, proxy, unknown} and fixes the unit of `actual`; `in_range` is the verdict computed from `human ?? proxy` (use `n/a (proxy unit)` when only a proxy exists); `observed_divisors` is a `;`-separated `class=÷N` list in AI-assisted mode, else `—`. Epics record one rolled-up row (summed step actuals in matching units) in addition to each step's own row.
