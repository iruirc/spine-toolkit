...
Step-task triggers cover sub-tasks of an epic: "create sub-task for 052", "step for 052", "shag for epic 052" — including named steps ("create step address-form for 052").
...
6. **Locate the template** (read the first existing path):
   a. `<core-root>/templates/task-md/task-root.md`
...
9. **Optional override lines** — append via Edit immediately after the `[NEED_REVIEW]` line, one per line, only where the user explicitly asked for something other than the project default. Each one overrides the field of the same name in `CLAUDE-spine-toolkit.md`. Otherwise leave the template as is (the commented-out lines stay as documentation).
   ...
   - `[WALKTHROUGH_CHECK] = [<auto|on|off>]` — write `off` when the user does not want this task's `Walkthrough.md` read back by a fresh agent, or `on` to have a `brief` one read back too; `auto`, the default, reads it back at `deep` only. Never written for `REVIEW` or `RESEARCH`, where the profile writes no walkthrough.
   - `[SECURITY] = [<auto|on|off>]` — write `on` when the user wants the security lens on this task whatever it touches, `off` when they said it needs none; otherwise leave it out and let the triage decide. Never written for `REVIEW`, `RESEARCH` or `TEST`, which run no lens.
...
1. **Identify the parent** — by number ("for 052"), slug ("for checkout-redesign"), or current conversation context. Ambiguous → ask the user via the structured question mechanism using key `ambiguous_parent_question` with placeholder `{candidates}` (the matching folder names).
2. **Find the parent folder**: `052-*` in any STATUS folder (explicit per-status `ls Tasks/<STATUS>/`; a recursive glob may not follow a symlinked `Tasks/`).
