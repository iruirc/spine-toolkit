...
- "создай под-задачу для 052", "step для 052", "шаг к эпику 052"
- Named step: "создай шаг address-form для 052"
...
1. **Identify parent** — by number ("для 052"), slug ("для checkout-redesign"), or current conversation context. Ambiguous → ask.
2. **Find parent folder**: `Tasks/**/052-*` (any STATUS).
3. **Choose step name**:
   - Numeric: find max existing `N.step` in the parent (including nested epics siblings), increment by 1 → `<N+1>.step`
   - Named: user said "шаг address-form" → `address-form.step`
