...
1. **Identify the parent** — by number ("for 052"), slug ("for checkout-redesign"), or current conversation context. Ambiguous → ask the user via `AskUserQuestion` using key `ambiguous_parent_question` with placeholder `{candidates}` (the matching folder names).
