...
- **Research** — `[architect]`, after the `[security]` lens wherever the `security-lens` skill runs it; the architect folds its findings into the risks, or records the verdict line that stands in for them. Artifact: `Research.md` in the task folder. Goal: investigate the domain, surface risks, propose architectural options.
...
- **Execute** — `[developer]` + `[tester]` (if `need_test=true` in args). Implements the phases from `Plan.md` step by step, updating both progress layers as work proceeds. **MUST create one git commit per green phase** — autonomously, without a user prompt. Under `fix-review` it runs the single "Review fixes <n>" phase built from `fix_findings` instead of `Plan.md`'s phases (see Done).
...
- **The security lens runs before Plan instead**, wherever the `security-lens` skill runs it: its
  findings, or the verdict line that stands in for them, go into the `## Research` section.
