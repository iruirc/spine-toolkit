...
description: |
  REVIEW profile workflow: a single pass by the reviewer role's agent plus auto-move into DONE/ on APPROVED. Activated by spine-toolkit:orchestrator; not invoked by the user directly.
...
The field structure is documented in `spine-toolkit:orchestrator` (section **Outbound Contract**). Workflow-review accepts every field already filled — invariant.
...
- `need_test`, `need_review` — ignored: this profile IS the review; `[reviewer]` does not call itself recursively, and tests run inside the donor workflows (FEATURE/BUG/REFACTOR/TEST).
...
- **Auto-move** — a deterministic post-stage. The workflow reads the first line of `Review.md` strictly as the field `[REVIEW_STATUS] = <value>` (regex: starts with `[REVIEW_STATUS] =`, value parsed by `=`). Substring search inside the body of `Review.md` is forbidden. Possible values and actions:
  - `APPROVED` → call `spine-toolkit:task-move` to relocate the task folder into `Tasks/DONE/`. If the task is already in `Tasks/DONE/` — leave it (idempotent).
...
- `status=error` — a contract error (e.g. invalid `start_stage`, missing/invalid `[REVIEW_STATUS]`, fatal failure of `[reviewer]` or `task-move`).
