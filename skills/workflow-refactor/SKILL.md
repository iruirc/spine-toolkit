...
- **Analyze** — `[architect]`, after the `[security]` lens wherever the `security-lens` skill runs it; the architect folds its findings into the risks and adds every security property the move must keep to the invariant, or records the verdict line that stands in for the findings. Artifact: `Research.md` describing the current state (what is bad, why, what risks the refactor carries), a map of affected components, and the target state. Goal: refactor **without changing external behavior** — only structure, readability, maintainability, type/module boundaries, naming, and dependency isolation change. The public API/behavior contract is preserved as an invariant.
...
- **Refactor** — `[refactorer]`, or `[tester]` for a phase that only adds or changes tests. Applies the refactor phase by phase from `Plan.md`, updating both progress layers as work proceeds. **MUST create one git commit per green phase** — autonomously, without a user prompt. Under `fix-review` it runs the single "Review fixes <n>" phase built from `fix_findings` instead of `Plan.md`'s phases (see Done).
...
- **Done** — final report `Done.md`: what was refactored, why it is now better (readability, separation of concerns, reduced coupling), measurable metrics where available (file size, cyclomatic complexity of key functions, dependency count), validation status (build/test result), and objections (if the user insisted on a contested decision). When `Done.md` already exists — a redo, a restart, a continue after a hand-back — every claim in it is checked against the current artifacts and the `git log` of every repository the task touched before it is kept, and the report says how many claims were corrected, or that every claim held. Before anything else Done closes every item under `## For Done` in `Review.md` by editing the task's own files, never code, and lists each under `## Review findings closed`; the first lines of `Done.md` are the `[DONE_COMMIT]` lines `scripts/task-ranges.sh` prints under `tips --kind done`. In a `catch-up` — the orchestrator's action for commits that landed after `Done.md`, running Validation → Review → Done over `review_ranges` — Done also appends to `Plan.md` a phase "Catch-up: commits after Done" that names the ranges, marked ✅, with a `**Verification:**` line naming the depth Validation ran at. Under `fix-review` — the orchestrator's action after Review returns `CHANGES_REQUESTED` — the code-changing stage runs one phase, "Review fixes <n>", built from `fix_findings`: appended to `Plan.md` first with a `**Verification:**` line, then fixed and committed; Validation, Review of those commits and Done follow as usual, and with `after_done` Done also writes the catch-up phase.
  ...
  Refresh `Walkthrough.md` here when the Refactor stage did not run in this invocation — an entry at Review or Done means commits landed after it was written. `task-walkthrough` owns the refresh rules; its `[COVERS]` line decides whether there is anything to do. A refresh that changed the file is checked the same way.
...
- **Analyze does not get its own stage.** No agent is dispatched for it and no `Research.md` is
  ...
  That section is `Research.md` folded into `Plan.md`, so the `task-documents` skill's `Research.md`
  section applies to it.
- **The security lens runs before Plan instead**, wherever the `security-lens` skill runs it: its
  findings, or the verdict line that stands in for them, go into the `## Analysis` section.
