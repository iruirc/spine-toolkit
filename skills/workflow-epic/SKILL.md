...
- **Plan** — `swift-toolkit:swift-architect`. **Two branches:**
  ...
  **Branch A — Decomposition.**
  ...
  - Step folders are created physically: `Tasks/<STATUS>/<epic-id>-<slug>/1.step/`, `2.step/`, …, `address-form.step/` (any name with the `.step` suffix). Each contains its own `Task.md`. Creating the physical folders is the responsibility of `swift-toolkit:task-new` (see section 6).
