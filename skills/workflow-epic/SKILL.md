...
- **Plan** — `[architect]`. **Two branches:**
  ...
  **Branch A — Decomposition.**
  ...
  - Each step's `Task.md` is written through `task-new` by applying the `task-documents` skill, its section on a step's `Task.md`: the problem on one example, the epic's decisions this step does not reopen, and the three anchors `### Expected behaviour`, `### Questions for Research`, `### Acceptance`. The orchestrator measures them after this stage with `scripts/lint-artifact-budget.sh --task-docs` against the `Task.md` ceiling in the contract's `budgets` field.
  - The plan applies the `task-documents` skill to `Plan.md`: each step in plain words, with what it waits for and what is true after it, why the order is what it is, a risk as what a user would see, and what happens when a step fails.
