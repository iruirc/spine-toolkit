...
- `start_phase` — for EPIC means the **`step_id`** (e.g. `start_phase=2.step` or `start_phase=address-form.step`), not a phase inside a stage. Research/Plan stages are not decomposed into the usual phases — the resume unit inside Execute is a step.
...
- `lang` — project language for artifact prose + the final report; artifact structure (headings, field labels, status enums) stays EN. See `conventions/i18n.md` → "Artifact authoring rule". Passed through to Research/Plan subagents and inherited by every step workflow-*. Every subagent prompt names it in words (`English`, `Russian`) at its start and again as its last line.
...
- `plugin_root` — the expanded absolute path of the plugin root, passed to every profile under Method A (orchestrator → Outbound Contract). For EPIC it is also what the fallback builds a path from when a step workflow's name does not resolve, since the workflow sandbox cannot expand `${CLAUDE_PLUGIN_ROOT}` itself. Absent, the script refuses before any stage runs. Method B ignores it.
...
- **Plan** — `[architect]`. **Two branches:**
  ...
  **Branch A — Decomposition.**
  ...
  - Each step is described as a separate task: it has its own `[TASK_TYPE]` (FEATURE/BUG/REFACTOR/TEST/EPIC/RESEARCH — yes, recursive EPIC and RESEARCH are allowed; never QUICK, which only the user chooses: a Plan that returns a QUICK step retypes it once in its `Task.md` and `Plan.md`, and stops the epic before Execute if QUICK is still there, as `workflows/profile-epic.js` does), its own `[STATUS]` ∈ {PENDING, IN_PROGRESS, DONE, DEFERRED, BLOCKED, SKIPPED} (TODO and ACTIVE are the pre-vocabulary spellings of PENDING and IN_PROGRESS and are read as them), an optional `[WORKFLOW_MODE]`, and its own `## 4. [Stack]` (or inherits from the epic).
  - Each step's `Task.md` is written through `task-new` by applying the `task-documents` skill, its section on a step's `Task.md` — it holds what goes above the anchors `### Expected behaviour`, `### Questions for Research`, `### Acceptance` and under each. The orchestrator measures them after this stage with `scripts/lint-artifact-budget.sh --task-docs` against the `Task.md` ceiling in the contract's `budgets` field.
  - The plan applies the `task-documents` skill, its `Plan.md` section — it holds how a step is described, how a risk is written, and what the plan leaves out.
  - Step folders are created physically: `Tasks/<STATUS>/<epic-id>-<slug>/1.step/`, `2.step/`, …, `address-form.step/` (any name with the `.step` suffix). Each contains its own `Task.md`. Creating the physical folders is the responsibility of `spine-toolkit:task-new` (see section 6).
...
- **Done** — final report `Done.md` for the epic:
  ...
  - When `Done.md` already exists — a redo, a restart, a continue after a hand-back — every claim in it is checked against the current artifacts and the `git log` of every repository the task touched before it is kept, and the report says how many claims were corrected, or that every claim held.

  On branch A, and only for a walk with no failed, cancelled or pending steps, also apply `spine-toolkit:task-walkthrough` and write `Walkthrough.md` — the value arrives in the contract as `walkthrough`; `conventions/task-settings.md` holds the chain it was resolved along. It is the depth the contract carries — `brief` or `deep` — and `task-walkthrough` `## The switch` says what each one changes. At `deep` the step stands in for the commit — a glossary, the order of the steps, and a section per step. An epic has no commits of its own: the file sits a layer above the steps, linking to each step's own walkthrough rather than restating it. When the contract's `walkthrough_check` is `on` and the write changed the file, `task-walkthrough` → `## Check` follows it: a fresh agent of the writer's role reads the file alone, and the writer revises once from what that reader could not follow. Branch B gets none — there is no implementation to walk through.
