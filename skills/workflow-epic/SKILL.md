...
- **Plan** — `[architect]`. **Two branches:**
  ...
  **Branch A — Decomposition.**
  ...
  - Each step is described as a separate task: it has its own `[TASK_TYPE]` (FEATURE/BUG/REFACTOR/TEST/EPIC/RESEARCH — yes, recursive EPIC and RESEARCH are allowed; never QUICK, which is for a root task the user chose, and a step carrying it stops Execute before any step runs), its own `[STATUS]` ∈ {PENDING, IN_PROGRESS, DONE, DEFERRED, BLOCKED, SKIPPED} (TODO and ACTIVE are the pre-vocabulary spellings of PENDING and IN_PROGRESS and are read as them), an optional `[WORKFLOW_MODE]`, and its own `## 4. [Stack]` (or inherits from the epic).
